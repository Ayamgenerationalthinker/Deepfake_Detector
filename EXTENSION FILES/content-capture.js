...
function captureVideoFrames() {
  ...
  return new Promise(async (mainResolve, mainReject) => {
    try {
      ...
      // Detect the largest face so local inference can use a face crop like training.
      const detectFaces = async (imgData) => {
        if (!faceDetectionSupported || !faceDetector) {
          return { hasFaces: false, faceCount: 0, box: null };
        }
        ...
        try {
          ...
          return {
            hasFaces: Boolean(largestFace),
            faceCount: faces.length,
            box: largestFace ? largestFace.boundingBox : null
          };
        } catch (e) {
      ...
      const cropFaceForInference = (imgData, box) => {
        return new Promise((resolve) => {
          const img = new Image();
          img.onload = () => {
            // The training pipeline uses square 224px face crops. Keep the
            // same geometry in the browser, including when FaceDetector is unavailable.
            const sourceWidth = img.naturalWidth;
            const sourceHeight = img.naturalHeight;
            const detectedWidth = box ? box.width : Math.min(sourceWidth, sourceHeight) * 0.7;
            const detectedHeight = box ? box.height : detectedWidth;
            const side = Math.min(
              Math.max(detectedWidth, detectedHeight) * 1.4,
              sourceWidth,
              sourceHeight
            );
            const centerX = box ? box.x + box.width / 2 : sourceWidth / 2;
            const centerY = box ? box.y + box.height / 2 : sourceHeight / 2;
            const left = Math.max(0, Math.min(sourceWidth - side, centerX - side / 2));
            const top = Math.max(0, Math.min(sourceHeight - side, centerY - side / 2));
            const cropCanvas = document.createElement('canvas');
            cropCanvas.width = 224;
            cropCanvas.height = 224;
            const cropContext = cropCanvas.getContext('2d');
            cropContext.drawImage(img, left, top, side, side, 0, 0, 224, 224);
      ...
      for (let i = 0; i < initialFrameCount; i++) {
        try {
          ...
          if (!frameHashes.has(result.hash)) {
            const faceResult = faceDetectionSupported
              ? await detectFaces(result.frame)
              : { hasFaces: false, faceCount: 0, box: null };
            const inferenceFrame = await cropFaceForInference(result.frame, faceResult.box);
            frames.push(inferenceFrame);
                ...
                try {
                  ...
                  if (!result.skipped && !frameHashes.has(result.hash)) {
                    const faceResult = faceDetectionSupported
                      ? await detectFaces(result.frame)
                      : { hasFaces: false, faceCount: 0, box: null };
                    frames.push(await cropFaceForInference(result.frame, faceResult.box));
    ...
    instructions.innerHTML = `
      <div class="df-instructions-content">
        ...
        <div style="margin-top: 20px;">
          <button class="df-continue-btn" style="background: #06B6D4; color: white; border: none; padding: 8px 16px; border-radius: 4px; margin-right: 10px; cursor: pointer;">Continue Capture</button>
      ...
      async function proceedToWebRTC(captureResult, videoElement) {
        ...
        try {
          ...
          message.innerHTML = `
            ...
            <div style="margin-top: 20px;">
              <button class="df-continue-btn" style="background: #06B6D4; color: white; border: none; padding: 10px 20px; border-radius: 4px; cursor: pointer;">Continue with Screen Capture</button>
